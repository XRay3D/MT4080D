    ...
    , m_model(new MyModel(this))
{
    ...
    connect(reinterpret_cast<MyHeader*>(horizontalHeader()), &MyHeader::stateChanged, [&](const UsedMap& enabled, int /*orientation*/) {
    ...
    QAction* at = menu.addAction(
        QIcon(), "Копировать данные", [&]() {
            ...
            for (int row = 0; row < 58; ++row) {
                for (int col = 0; col < columnCount; ++col) {
                    str.append(QString::number(m_model->getChData(row, col), 'g', 7) + '\t');
                }
                str.append("\r\n");
            }
...
void MyHeader::paintSection(QPainter* painter, const QRect& rect, int logicalIndex) const
{
    ...
    m_isOn.at(logicalIndex) ? option.state = QStyle::State_On : option.state = QStyle::State_Off;
